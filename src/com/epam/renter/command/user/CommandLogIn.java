...
public class CommandLogIn implements ICommand {
	...
	public String execute(HttpServletRequest request,
		...
		User user = DAOFactory.getDAOUser().findByLogin(login);

		if (user == null) {
			logger.info(String.format("Wrong login. Login=%s", login));
			session.setAttribute(ERROR, Message.WRONG_LOGIN);
			...
			logger.info(String.format("Wrong password. Login=%s, Password=%s",
					login, password));
			session.setAttribute(ERROR, Message.WRONG_PASSWORD);
			session.setAttribute(LAST_PAGE,
					Config.getInstance().getProperty(Config.ERROR_LOGIN));
			request.getRequestDispatcher(
					Config.getInstance().getProperty(Config.ERROR_LOGIN))
		...
		// login is OK, password is OK
			...
			Admin admin = DAOFactory.getDAOAdmin().findByUserID(user.getId());
			...
			if (admin != null) {
				...
				session.setAttribute(ADMIN_MESSAGE, Message.ADMIN_WELCOME);
				session.setAttribute(LAST_PAGE, Config.getInstance()
						.getProperty(Config.ADMIN_MESSAGE));
				request.getRequestDispatcher(
